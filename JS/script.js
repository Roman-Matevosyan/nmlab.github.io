...
(function () {
    ...
    const flag_image_block_img = $('.flag-image-block img');
	...
	languageArr.forEach((el, i) => {
		if (flagChange === el) { saveLocalStorage(el, i); }
		$(`#${el}`).on('click', () => { saveLocalStorage(el, i); });
	});

    function hideCurrentFlag(currentImgSrc) {
        flag_images.forEach((el, i) => {
            if (el === currentImgSrc) {
                $(flag_image_block_img[i]).css('display', 'none')
            } else {
                $(flag_image_block_img[i]).css('display', 'block')
            }
        })
    }
	...
	function saveLocalStorage (text_in_language, imgIndex) {
		...
		hideCurrentFlag(`${flag_images[imgIndex]}`)

		languageArr.forEach((el, i) => {
			if (text_in_language === el) {
                text_languages[i].forEach((el, j) => {
